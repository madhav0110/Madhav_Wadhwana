  ...
  return (
    <section id="about" className="section-shell section-pad">
      ...
      <div className="mt-12 grid gap-4 md:grid-cols-3">
        {cards.map((card, index) => <Reveal key={card.title} delay={index * 0.08} className="premium-card rounded-3xl p-6"><card.icon className="mb-6 text-cyan-200" /><h3 className="text-xl font-semibold">{card.title}</h3><p className="mt-3 leading-7 text-slate-400">{card.copy}</p></Reveal>)}
      </div>
    </section>
  )
}
